...
class StrictSurge {
  final double? sts, prewake;
  const StrictSurge({this.sts, this.prewake});
}
...
Future<BpStats> computeBpStats({
  required List<BpPoint> points,
  required Health health,
  required SurgeParams params,
}) async {
  ...
  double? mean(List<double> a) =>
      a.isEmpty ? null : a.reduce((x, y) => x + y) / a.length;
  final dayMeanS = mean(dayS), nightMeanS = mean(nightS);
  final dayMeanD = mean(dayD), nightMeanD = mean(nightD);
  double? dip(double? day, double? night) =>
      (day != null && night != null && day > 0)
      ? ((day - night) / day * 100.0)
      : null;
  final dipS = dip(dayMeanS, nightMeanS);
  final dipD = dip(dayMeanD, nightMeanD);
  final morningSurgeS = (mean(morningS) != null && mean(earlyS) != null)
      ? (mean(morningS)! - mean(earlyS)!)
      : null;
  final morningSurgeD = (mean(morningD) != null && mean(earlyD) != null)
      ? (mean(morningD)! - mean(earlyD)!)
      : null;

  final strict = await _computeMorningSurgeStrict(points, health, params);
  return BpStats(
    dayMeanS: dayMeanS,
    nightMeanS: nightMeanS,
    dayMeanD: dayMeanD,
    nightMeanD: nightMeanD,
    dipS: dipS,
    dipD: dipD,
    morningSurgeS: morningSurgeS,
    morningSurgeD: morningSurgeD,
    morningSurgeStrictSts: strict.sts,
    morningSurgeStrictPrewake: strict.prewake,
  );
}

Future<StrictSurge> computeStrictWithWakeTimes(
  List<BpPoint> points,
  Map<DateTime, DateTime> wakes,
  SurgeParams p,
) async {
  ...
  byDay.forEach((day, list) {
    ...
    for (final e in list) {
      final t = e.t!;
      final v = e.sbp!;
      if (!t.isBefore(morningStart) && !t.isAfter(morningEnd)) {
        maxMorning = (maxMorning == null) ? v : math.max(maxMorning, v);
      }
      if (!t.isBefore(troughStart) && !t.isAfter(troughEnd)) {
        minTrough = (minTrough == null) ? v : math.min(minTrough, v);
      }
      if (!t.isBefore(preStart) && !t.isAfter(preEnd)) {
        sumPre += v;
        nPre++;
      }
    }
    if (maxMorning != null && minTrough != null) {
      stsSum = (stsSum ?? 0) + (maxMorning - minTrough);
      stsN++;
    }
    if (maxMorning != null && nPre > 0) {
      preSum = (preSum ?? 0) + (maxMorning - (sumPre / nPre));
      preN++;
    }
  });
  return StrictSurge(
...
Future<StrictSurge> _computeMorningSurgeStrict(
  List<BpPoint> s,
  Health health,
  SurgeParams params,
) async {
...
Future<Map<DateTime, DateTime>> _inferWakeTimesStrict(
  Health health,
  DateTime start,
  DateTime end,
  SurgeParams p,
  ...
  try {
    ...
    for (final pt in sleep) {
      final d = DateTime(pt.dateTo.year, pt.dateTo.month, pt.dateTo.day);
      (byDay[d] ??= []).add(pt);
    }
    for (final e in byDay.entries) {
      ...
      for (final pnt in e.value) {
        ...
        if (lt.hour >= p.wakeEarliestHour && lt.hour <= p.wakeLatestHour) {
  ...
  void setIfEmpty(DateTime key, DateTime value) {
  ...
  try {
    ...
    for (final pnt in steps) {
      final from = pnt.dateFrom.toLocal();
      final to = pnt.dateTo.toLocal();
      double val = _toDouble(pnt.value) ?? 0;
      DateTime cur = from;
  ...
  try {
    ...
    for (final e in byDay.entries) {
      ...
      for (int i = 0; i < points.length; i++) {
        final t = points[i].t;
        if (t.hour < p.wakeEarliestHour + 2) continue;
        double sum = 0;
        int n = 0;
        ...
        if (n > 0 && (sum / n) >= baseline + p.hrRiseBpm) {
          setIfEmpty(e.key, t);
          break;
        }
      }
...
class _SimpleSample {
  final DateTime t;
  final double v;
  _SimpleSample({required this.t, required this.v});
}
