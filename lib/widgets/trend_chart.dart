...
class TrendChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final times = series.where((e) => e.t != null).map((e) => e.t!).toList()..sort();
    if (times.isNotEmpty) {
    ...
    if (distribution) {
      ...
      while (!cur.isAfter(last)) { days.add(cur); cur = cur.add(const Duration(days: 1)); }
      ...
      for (int i = 0; i < days.length; i++) {
        final d = days[i]; final list = byDay[d];
        if (list != null && showSys) {
          final vals = list.where((e) => e.sbp != null).map((e) => e.sbp!).toList()..sort();
          if (vals.isNotEmpty) { mSys[i] = _q(vals,0.5); p25Sys[i] = _q(vals,0.25); p75SysL[i] = _q(vals,0.75); }
        }
        if (list != null && showDia) {
          final vals = list.where((e) => e.dbp != null).map((e) => e.dbp!).toList()..sort();
      ...
      void interp(List<double?> a) {
        ...
        while (i < n) { if (a[i] != null) { i++; continue; } int j = i; while (j < n && a[j] == null) { j++; }
          double? left = i > 0 ? a[i - 1] : null; double? right = j < n ? a[j] : null;
          for (int k = i; k < j; k++) {
            if (left != null && right != null) { double t = (k - (i - 1)) / (j - (i - 1)); a[k] = left * (1 - t) + right * t; }
            else if (left != null) { a[k] = left; } else if (right != null) { a[k] = right; }
          }
          i = j;
        }
      }
      ...
      List<double?> smoothMA(List<double?> a, int halfWin) { final n = a.length; final out = List<double?>.filled(n, null);
        for (int i = 0; i < n; i++) { int s = (i - halfWin).clamp(0, n - 1); int e = (i + halfWin).clamp(0, n - 1); double sum = 0; int c = 0; for (int k = s; k <= e; k++) { final v = a[k]; if (v != null) { sum += v; c++; } } out[i] = c > 0 ? sum / c : a[i]; }
        return out; }
      List<double?> smoothEMA(List<double?> a, int windowDays) {
        final n = a.length; final alpha = 2 / (windowDays + 1); final f = List<double?>.filled(n, null); double? prev;
        for (int i = 0; i < n; i++) { final v = a[i]; if (v == null) { f[i] = prev; continue; } prev = (prev == null) ? v : (alpha * v + (1 - alpha) * prev); f[i] = prev; }
        final b = List<double?>.filled(n, null); prev = null;
        for (int i = n - 1; i >= 0; i--) { final v = a[i]; if (v == null) { b[i] = prev; continue; } prev = (prev == null) ? v : (alpha * v + (1 - alpha) * prev); b[i] = prev; }
        final out = List<double?>.filled(n, null);
        for (int i = 0; i < n; i++) { final x = f[i]; final y = b[i]; if (x != null && y != null) { out[i] = (x + y) / 2; } else { out[i] = x ?? y ?? a[i]; } }
        return out; }
      if (smoothingEnabled) {
        int half = ((smoothingWindowDays.clamp(1, 31)) - 1) ~/ 2; if (half < 1) half = 1;
    ...
    final lines = <LineChartBarData>[]; final between = <BetweenBarsData>[];
    if (!distribution) {
      if (showSys) lines.add(LineChartBarData(spots: sysSpots, isCurved: false, color: Colors.red, barWidth: 2, dotData: const FlDotData(show: true)));
      if (showDia) lines.add(LineChartBarData(spots: diaSpots, isCurved: false, color: Colors.blue, barWidth: 2, dotData: const FlDotData(show: true)));
    } else {
      if (showSys) {
        lines.add(LineChartBarData(spots: q25Sys, isCurved: true, color: Colors.transparent, barWidth: 0, dotData: const FlDotData(show: false)));
        lines.add(LineChartBarData(spots: q75Sys, isCurved: true, color: Colors.transparent, barWidth: 0, dotData: const FlDotData(show: false)));
        lines.add(LineChartBarData(spots: q50Sys, isCurved: true, color: Colors.red, barWidth: 2, dotData: const FlDotData(show: false)));
        between.add(BetweenBarsData(fromIndex: lines.length - 3, toIndex: lines.length - 2, color: const Color(0x26F44336)));
      }
      if (showDia) {
        lines.add(LineChartBarData(spots: q25Dia, isCurved: true, color: Colors.transparent, barWidth: 0, dotData: const FlDotData(show: false)));
        lines.add(LineChartBarData(spots: q75Dia, isCurved: true, color: Colors.transparent, barWidth: 0, dotData: const FlDotData(show: false)));
        lines.add(LineChartBarData(spots: q50Dia, isCurved: true, color: Colors.blue, barWidth: 2, dotData: const FlDotData(show: false)));
        between.add(BetweenBarsData(fromIndex: lines.length - 3, toIndex: lines.length - 2, color: const Color(0x1F2196F3)));
      }
    }
    ...
    if (secondary.isNotEmpty) {
      final ys = <double>[];
      for (final p in secondary) { ys.add(p.value); }
    ...
    final minY = _autoMinY([if (showSys) sysSpots, if (showDia) diaSpots, secSpots]);
    final maxY = _autoMaxY([if (showSys) sysSpots, if (showDia) diaSpots, secSpots]);
      ...
      LineChartData(
        ...
        titlesData: FlTitlesData(
          ...
          bottomTitles: AxisTitles(
            sideTitles: SideTitles(showTitles: true, interval: (maxX / 5).clamp(1, 30), getTitlesWidget: (value, meta) {
              final d = effStart.add(Duration(days: value.round()));
              return SideTitleWidget(meta: meta, child: Text('${d.month}/${d.day}'));
            }),
            axisNameWidget: const Text('Date'), axisNameSize: 16,
          ),
          topTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
          rightTitles: AxisTitles(
            sideTitles: secondary.isEmpty ? const SideTitles(showTitles: false) : SideTitles(
              showTitles: true,
              reservedSize: 40,
  ...
  double _autoMinY(List<List<FlSpot>> lists) {
    double m = 300;
    for (final l in lists) { for (final p in l) { if (p.y < m) m = p.y; } }
    if (m == 300) m = 40;
